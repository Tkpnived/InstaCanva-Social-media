...
from .models import logindb,Message,updb,follows,like,postm,Status
...
def web(request):
    ...
    return render(request, "insta.html", {"datas": datas,
...
def search(request):
    ...
    if 'name' in request.session:
        ...
        if request.session.get('name'):
            user_object = logindb.objects.get(username=username)
    return render(request, 'search.html', {"data":data,"user_object": user_object})
...
def message(request, dataid):
    if 'name' in request.session:
        ...
        user_objects = logindb.objects.filter(username=dataid)
        user_object = logindb.objects.get(username=username)
        ...
        return render(request, 'messages.html', {"user_object":user_object,"user_objects": user_objects, "data": data, "datas": datas, "all_msgs": all_msgs, "unique_names": unique_names})
...
def person(request, dataid, item):
    if 'name' in request.session:
        ...
        return render(request, "persons.html", {"user_object":user_object, "user_objects": user_objects, "data": data, "datas": datas, "followings": followings, "is_following": is_following})
...
def foll(request,dataid,item):
    ...
    return render(request,"showfollow.html",{"user_object":user_object,"sta":sta,"stase":stase})
...
def followers(request,dataid,item):
    if 'name' in request.session:
        ...
        user_object = logindb.objects.get(username=username)
        stase = logindb.objects.filter(username=item)
    return render(request,"followers.html",{"user_object":user_object,"sta":sta,"stase":stase})
...
def mes(request,dataid):
    ...
    return render(request,"yourmessages.html",{"user_object":user_object,"datas": datas, "unique_names": unique_names})
